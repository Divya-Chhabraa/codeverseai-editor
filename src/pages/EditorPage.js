...
const EditorPage = () => {
    ...
    // Enhanced message to dashboard with more details
    ...
    attemptTabClose(token);
}

function attemptTabClose(token) {
    console.log('Attempting to close editor tab...');
    
    // Strategy 1: Direct close (works for windows opened via window.open())
    try {
        window.close();
    } catch (error) {
        console.log('Direct close failed:', error);
    }
    
    // Strategy 2: If still open after short delay, try redirect + close
    setTimeout(() => {
        if (!window.closed) {
            console.log('Tab still open, trying redirect strategy...');
            // Redirect to dashboard
            window.location.href = `https://codeverseai.streamlit.app/?page=dashboard&token=${token}&editor_closed=true`;
            
            // Try closing after redirect
            setTimeout(() => {
                if (!window.closed) {
                    try {
                        window.close();
    ...
    // Strategy 3: Final fallback - just stay on dashboard
    setTimeout(() => {
        if (!window.closed) {
            console.log('Using final fallback - staying on dashboard');
            // Already redirected in strategy 2, so nothing needed here
        }
    }, 2000);
}
...
window.addEventListener('beforeunload', () => {
    // Send closure message if possible
    if (window.opener && !window.opener.closed) {
        const token = localStorage.getItem("accessToken");
        const roomId = localStorage.getItem('currentRoom');
